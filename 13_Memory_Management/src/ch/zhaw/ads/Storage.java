...
public class Storage {
    ...
    private static void mark(Collectable cObject) {
        ...
        if (((CObject) cObject).next != null && !((CObject) cObject).next.isMarked()){
            mark(((CObject) cObject).next);
        }
        // Aufgabe 13.1
    }
    ...
    private static void sweep() {
        ...
        for (Collectable collectable : getOldHeap()){
            if (collectable.isMarked()) collectable.setMark(false);
            else {
                if(!youngGenerationOnly)delete(collectable);
            }
        }
    }
